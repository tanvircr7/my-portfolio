...
export default async function ProjectPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <PageContainer>
      <Link href="/projects" className="text-sm text-foreground/60 hover:text-foreground">← Back to projects</Link>
      ...
      <div className="mt-8 flex flex-wrap gap-2 text-xs">
        {project.tags.map(tag => (
          <span key={tag} className="px-2 py-0.5 rounded-full bg-foreground text-background font-medium">{tag}</span>
        ))}
      </div>
